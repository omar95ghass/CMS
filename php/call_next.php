...
$stmt = $conn->prepare("SELECT id, number, clinic FROM queue WHERE user_id = ? AND status = 'waiting' AND date = ? ORDER BY id DESC LIMIT 1");
...
if ($result->num_rows > 0) {
    ...
    echo json_encode(['status' => 'success', 'number' => $number, 'id' => $id, 'clinic' => $clinic]);
} else {
